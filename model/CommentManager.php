...
class CommentManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $comments = $db->prepare('SELECT comments.id, comments.user_id, comments.comment, DATE_FORMAT(comments.comment_date, \'%d/%m/%Y à %Hh%i\') AS comment_date_fr, comments.moderation, users.pseudo FROM comments INNER JOIN users ON comments.post_id = ? AND users.id = user_id ORDER BY comment_date DESC');
